...
int main(int argc, char **argv) {
  ...
  long long words, size, a, b, c, d,e,ff,words_tag, size_tag, from_word_num, from_word_dim, to_word_num, to_word_dim;
  real *M;
  real *N;
  real *from_vec;
  real *to_vec;
  ...
  while (fgets(line, sizeof(line), f_test) != NULL) {
    ...
    candicate_num = split(candicate, predict," ");
    ...
    for (c = 0; c < to_word_num; c++) {
      ...
      if (b == c) a = 1;
      if (a == 1) continue;
      for (e = 0; e < candicate_num; ++e){
        if (!strcmp(candicate[e],&to_vacab[c * max_w])){
          exist=1;     
          break;
        }
      }
      if (exist==1){
        // printf("%s---%s---%s\n",user_id,candicate[e],&to_vacab[c * max_w]);
        dist = 0;
    ...
    for (e = 0; e < candicate_num; ++e){
      int flag = 0;
      for (ff = 0; ff < hit_num; ++ff){
        if (!strcmp(candicate[e],bestw[ff])) //candicate没有出现
        {
          flag = 1;
          break;
        }
