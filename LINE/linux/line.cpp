...
char network_file[MAX_STRING], embedding_file[MAX_STRING],tag_file[MAX_STRING];
...
void InitTagEdgeAliasTable()
{
	...
	while (num_small_block && num_large_block)
	{
		...
		alias_tag[cur_small_block] = cur_large_block;
	...
	while (num_large_block) prob[large_block[--num_large_block]-num_edges] = 1;
	while (num_small_block) prob[small_block[--num_small_block]-num_edges] = 1;
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\nExamples:\n");
		printf("./line -train net.txt -train_tag_file tag.txt -output vec.txt -binary 1 -size 200 -order 2 -negative 5 -samples 100 -rho 0.025 -threads 20\n\n");
		return 0;
	}
