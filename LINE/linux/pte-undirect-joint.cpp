...
int *vertex_hash_table, *neg_table, *neg_table_tag;
int max_num_vertices = 1000, num_vertices = 0 ,num_vertices_user = 0 , num_vertices_tag = 0;
long long total_samples = 1, current_sample_count = 0, num_edges = 0 , tag_num_edges = 0,total_edges = 0;
...
long long *alias,*alias_tag;
double *prob,*prob_tag;
...
int AddVertex(char *name,int type,int source)
{
	int length = strlen(name) + 1;
	if (length > MAX_STRING) length = MAX_STRING;
	vertex[num_vertices].name = (char *)calloc(length, sizeof(char));
    vertex[num_vertices].type = type;
...
void ReadData()
{
	FILE *fin,*tag_file_fin;
	char name_v1[MAX_STRING], name_v2[MAX_STRING], str[2 * MAX_STRING + 10000];
	int vid;
	double weight;
	

	tag_file_fin = fopen(tag_file,"rb");
	if (tag_file_fin == NULL)
	{
		printf("ERROR: tag file not found!\n");
		exit(1);
	}
    ...
    tag_num_edges = 0;
	while (fgets(str, sizeof(str), tag_file_fin)) tag_num_edges++;
	fclose(tag_file_fin);
	printf("Number of tag edges: %lld          \n", tag_num_edges);

	total_edges = num_edges + tag_num_edges;

	edge_source_id = (int *)malloc(total_edges*sizeof(int));
	edge_target_id = (int *)malloc(total_edges*sizeof(int));
	edge_weight = (double *)malloc(total_edges*sizeof(double));
    ...
    tag_file_fin = fopen(tag_file,"rb");
	for (int k = num_edges; k != total_edges; k++)
	{

		fscanf(tag_file_fin, "%s %s %lf", name_v1, name_v2, &weight);
		// printf("tag_file name_v1：%s name_v2: %s weight：%lf \n",name_v1,name_v2,weight);

		if (k % 10000 == 0)
		{
			printf("Reading  tag edges: %.3lf%%%c", (k - num_edges) / (double)(tag_num_edges + 1) * 100, 13);
			fflush(stdout);
		}

		vid = SearchHashTable(name_v1);
		if (vid == -1) {
			vid = AddVertex(name_v1,0,1);
		}
		//检查是不是用户关系图中的顶点，如果是的话，将source改为2.
		if (vertex[vid].source==0)
		{
			vertex[vid].source = 2;
		}
...
/* The alias sampling algorithm, which is used to sample an edge in O(1) time. */
void InitTagEdgeAliasTable()
{
	alias_tag = (long long *)malloc(tag_num_edges*sizeof(long long));
	prob_tag = (double *)malloc(tag_num_edges*sizeof(double));
	if (alias_tag == NULL || prob_tag == NULL)
	{
		printf("Error: memory allocation failed!\n");
		exit(1);
	}

	double *norm_prob = (double*)malloc(tag_num_edges*sizeof(double));
	long long *large_block = (long long*)malloc(tag_num_edges*sizeof(long long));
	long long *small_block = (long long*)malloc(tag_num_edges*sizeof(long long));
	if (norm_prob == NULL || large_block == NULL || small_block == NULL)
	{
		printf("Error: memory allocation failed!\n");
		exit(1);
	...
	for (long long k = num_edges; k != total_edges; k++) sum += edge_weight[k];
	for (long long k = num_edges; k != total_edges; k++) norm_prob[k-num_edges] = edge_weight[k] * tag_num_edges / sum;
	for (long long k = total_edges - 1; k >= num_edges; k--)
	{
		if (norm_prob[k-num_edges]<1)
			small_block[num_small_block++] = k;
		else
			large_block[num_large_block++] = k;
	}
	...
	while (num_small_block && num_large_block)
	{
		cur_small_block = small_block[--num_small_block];
		cur_large_block = large_block[--num_large_block];
		prob_tag[cur_small_block-num_edges] = norm_prob[cur_small_block-num_edges];
		alias_tag[cur_small_block-num_edges] = cur_large_block;
		norm_prob[cur_large_block-num_edges] = norm_prob[cur_large_block-num_edges] + norm_prob[cur_small_block-num_edges] - 1;
		if (norm_prob[cur_large_block-num_edges] < 1)
			small_block[num_small_block++] = cur_large_block;
		else
			large_block[num_large_block++] = cur_large_block;
	}

	while (num_large_block) prob_tag[large_block[--num_large_block]-num_edges] = 1;
	while (num_small_block) prob_tag[small_block[--num_small_block]-num_edges] = 1;
...
long long SampleAnTagEdge(double rand_value1,double rand_value2)
{ 
    long long k = (long long)tag_num_edges * rand_value1;
    return rand_value2 < prob_tag[k] ? (k+num_edges):alias_tag[k];
}
...
void InitNegTable()
{
	double sum_relation = 0,sum_tag=0, cur_sum = 0, por = 0;
	int vid = 0, relation_index = 0, tag_index=0 , cnt_relation =0, cnt_tag =0;
	
	for (int k = 0; k != num_vertices; k++) {
		if (vertex[k].source ==0 || vertex[k].source ==2)
			cnt_relation +=1;
		if (vertex[k].source==1 || vertex[k].source ==2)
			cnt_tag +=1;
	}

	int * relationArray = (int *)malloc(cnt_relation * sizeof(int));
	int * tagArray = (int *)malloc(cnt_tag * sizeof(int));
	neg_table = (int *)malloc(neg_table_size * sizeof(int));
	neg_table_tag = (int *)malloc(neg_table_size * sizeof(int));

	for (int k = 0; k != num_vertices; k++) {
		if (vertex[k].source ==0 || vertex[k].source ==2){
			relationArray[relation_index++] =k;
		}
		if (vertex[k].source ==1 || vertex[k].source ==2){
			tagArray[tag_index++] =k;
		}
	}
	...
	for (int k = 0; k != neg_table_size; k++)
	{
		...
		{
			cur_sum += pow(vertex[relationArray[vid]].degree, NEG_SAMPLING_POWER);
			por = cur_sum / sum_relation;
			vid++;
		}
		neg_table[k] = relationArray[vid - 1];
	}
	vid =0;cur_sum=0;por=0;
	...
	for (int k = 0; k != neg_table_size; k++)
	{
		...
		neg_table_tag[k] = tagArray[vid - 1];
	}
}
...
void Output()
{
	...
	FILE *fo_tag = fopen(embedding_tag_file,"wb");
	for (int a = 0; a < num_vertices; a++)
	{
		if (vertex[a].type==0) //user
			num_vertices_user++;
		else   //tag
			num_vertices_tag++;
	}
	fprintf(fo, "%d %d\n", num_vertices_user, dim);
	fprintf(fo_tag, "%d %d\n", num_vertices_tag, dim);
	for (int a = 0; a < num_vertices; a++)
	{
		if (vertex[a].type==0)  //user
		{
			...
			fprintf(fo_tag, "%s ", vertex[a].name);
			if (is_binary) for (int b = 0; b < dim; b++) fwrite(&emb_vertex[a * dim + b], sizeof(real), 1, fo_tag);
			else for (int b = 0; b < dim; b++) fprintf(fo_tag, "%lf ", emb_vertex[a * dim + b]);
			fprintf(fo_tag, "\n");	
		}
	}
	fclose(fo);
...
void Output_Alias()
{
	printf("prob and alise\n");
	for (int a = 0; a < num_edges; a++)
	{
		printf("%d %lf %lld \n",a,prob[a],alias[a]);
	}

	printf("prob_tag and alise_tag\n");
	for (int a = 0; a < tag_num_edges; a++)
	{
		printf("%d %lf %lld \n",a,prob_tag[a],alias_tag[a]);
	}
}
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\t-train_tag_file <file>\n");
		printf("\t\tUse tag file data from <file> to tune the model\n");
		printf("\t-output <file>\n");
		...
		printf("\nExamples:\n");
		printf("./line -train net.txt -train_tag_file tag.txt -output vec.txt -output_tag vec_tag.txt -binary 1 -size 200 -order 2 -negative 5 -samples 100 -rho 0.025 -threads 20\n\n");
		return 0;
	}
	if ((i = ArgPos((char *)"-train", argc, argv)) > 0) strcpy(network_file, argv[i + 1]);
	if ((i = ArgPos((char *)"-train_tag_file", argc, argv)) > 0) strcpy(tag_file, argv[i + 1]);
	if ((i = ArgPos((char *)"-output", argc, argv)) > 0) strcpy(embedding_file, argv[i + 1]);
	if ((i = ArgPos((char *)"-output_tag", argc, argv)) > 0) strcpy(embedding_tag_file, argv[i + 1]);
